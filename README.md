...
- **LTF Ribbon Confirmation (Critical Bug Fix)**: Fixed a logic bug where Scalp trades ignored the actual LTF (15m) Mango Dynamic ribbon direction. Previously, if the Daily and HTF (4H/1H) trend were Bearish and the 15m candle was red, it would short even if the 15m ribbon was still Bullish. Scalps now strictly require the 15m ribbon to agree with the trade direction.
- **Balanced Entry Conditions & 2h Cooldown**: Fine-tuned the recent stringent filters. The chop filter was loosened to 0.3% (preventing TradFi indices like SPX and NDX from being excessively filtered) and the optimal entry zone was set to 75%. Also reduced the signal deduplication cooldown from 4 hours down to 2 hours, as 4 hours was unnecessarily blocking legitimate second-chance setups.
