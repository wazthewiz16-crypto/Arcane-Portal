...
- **Quality Over Quantity (Tightened Entries)**: Reduced daily signal volume from ~55 to a target of 10-20 by tightening core entry requirements. The "chop filter" minimum zone width was doubled to 0.4%, the optimal entry zone was restricted from 90% to 65% (avoiding edge-entries), and the bounce confidence bonus was nerfed from +15 to +8 to prevent inflating mediocre setups. 
- **Signal De-Duplication & 4h Cooldown**: Added a two-layer redundancy filter. First, the detector now suppresses multiple identical signals generated from different HTF combinations (e.g., 1H→15m and 4H→15m for the same asset) in a single run. Second, a hard 4-hour cooldown prevents the same asset and direction from firing again shortly after hitting TP or SL, eliminating re-entry spam.
